...
import { Sparkles, Lock, RefreshCw, ArrowLeft, Loader } from 'lucide-react';
...
export default function Tarot() {
  ...
  const startReading = async (spread: TarotSpread) => {
    ...
    if (spread.id === 'daily') {
      ...
      if (card) {
        const normalizedCard: TarotCard = {
          id: `daily-${card.number}`,
          number: card.number,
          name: card.name,
          symbol: card.symbol,
  ...
  if (spreadsLoading || cardsLoading) {
    return (
      <div style={{ 
        ...
        <div style={{ color: 'var(--text-muted)', marginTop: 12 }}>Загрузка...</div>
      </div>
    );
  }

  if (spreadsError || cardsError) {
    const errorText = spreadsError || cardsError || 'Ошибка сети';
    return (
      <div style={{ padding: 20, textAlign: 'center' }}>
        <p style={{ color: 'var(--error)', marginBottom: 16 }}>Ошибка загрузки тестов: {errorText}</p>
        <button
