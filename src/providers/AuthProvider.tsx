...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        if (parsed.isGuest) {
          ...
          const checkUser = async () => {
            if (isApiEnabled()) {
              ...
              if (u) {
                setUser({ ...parsed, email: u.email, name: u.name, birthDate: normalizeBirthDate(u.birthDate || parsed.birthDate) });
                return;
              }
            } else {
    ...
    if (isApiEnabled()) {
      ...
      if (result.ok && result.user) {
        const loggedInUser: User = {
          ...
          birthDate: normalizeBirthDate(result.user.birthDate),
          isGuest: false,
  ...
  const updateProfile = async (name: string, birthDate?: string): Promise<boolean> => {
    ...
    if (isApiEnabled()) {
      ...
      if (result.ok && result.user) {
        const updatedUser = { ...user, name: result.user.name, birthDate: normalizeBirthDate(result.user.birthDate) };
        setUser(updatedUser);
    ...
    if (success) {
      const updatedUser = { ...user, name, birthDate: normalizeBirthDate(birthDate) };
      setUser(updatedUser);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(updatedUser));
      return true;
    }
