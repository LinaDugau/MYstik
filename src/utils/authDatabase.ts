...
type PublicUser = { id: string; email: string; username: string; name: string; birthDate?: string };
...
export const authDatabase = {
  ...
  updateUser(id: string, data: { name?: string; birthDate?: string }): boolean {
    try {
      const db = getDatabase();
      const user = db.users.find((u) => u.id === id && u.is_guest === 0);
      if (!user) return false;
      if (typeof data.name === 'string') user.name = data.name.trim();
      if (typeof data.birthDate !== 'undefined') user.birth_date = data.birthDate;
      user.last_login = new Date().toISOString();
      saveDatabase(db);
      return true;
    } catch (_) {
  ...
  changePassword(id: string, oldPassword: string, newPassword: string): boolean {
    try {
      ...
      const user = db.users.find((u) => u.id === id && u.is_guest === 0);
      if (!user) return false;
      if (user.password_hash !== hashPassword(oldPassword)) return false;
