 ...
 * Если задан VITE_API_URL, используем его, иначе relative URL через фронтовый прокси.
 */
...
export async function apiGetUser(id: string): Promise<ApiUser | null> {
  try {
    const res = await fetch(buildApiUrl(`/api/user/${encodeURIComponent(id)}`));
    const data = await res.json().catch(() => ({}));
...
export async function apiChangePassword(
  ...
  try {
    const res = await fetch(buildApiUrl(`/api/user/${encodeURIComponent(id)}/change-password`), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
