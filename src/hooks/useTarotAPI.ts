...
export function useTarotSpreads() {
  ...
  useEffect(() => {
    const fetchSpreads = async () => {
      try {
        const response = await fetch(buildApiUrl('/api/tarot/spreads'));
...
export function useTarotCards() {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        const response = await fetch(buildApiUrl('/api/tarot/cards'));
        const data = await response.json();
...
export function useTarotReading() {
  ...
  const createReading = async (spreadId: string, cardIds: string[]): Promise<TarotReading | null> => {
    ...
    try {
      const response = await fetch(buildApiUrl('/api/tarot/reading'), {
        method: 'POST',
        headers: {
...
export function useUserTarotReadings(date?: string) {
  ...
  useEffect(() => {
    const fetchReadings = async () => {
      ...
      try {
        let url = buildApiUrl(`/api/user/${user.id}/tarot/readings`);
        if (date) {
          url += `?date=${date}`;
        }
